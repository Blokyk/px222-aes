...
lagrangeSBox :: Polynomial Byte
lagrangeSBox
    = foldl' (\acc x -> acc `add` ((sbox' !! Byte.asInt x) `multScalaire` f x)) one $ map byteFromInt [0..0xff]
    where
        f :: Byte -> Polynomial Byte
        f x = byteFromInt (invs !! Byte.asInt (computePolynomialAt l' x)) `multScalaire` l'
            where l' = foldl' (\acc sample -> acc `mult` nullifyAt (sbox' !! Byte.asInt sample)) one $ filter (/= x) $ map byteFromInt [0..0xff]
        nullifyAt :: Byte -> Polynomial Byte
        nullifyAt sample = polynomial [zero, one] `sub` polynomial [sample]
